...
def test_builder_default_cash_buy_future_sell_targets_gap_convergence():
    strategy = _build_builder_strategy(request(cash_side="BUY", future_side="SELL", target=2.0))
...
def test_builder_reverse_cash_sell_future_buy_uses_opposite_gap_direction():
    strategy = _build_builder_strategy(request(cash_side="SELL", future_side="BUY", target=2.0))
    assert strategy(point(-5.0), ()) == "BUY"
    assert strategy(point(-3.0), (point(-5.0),)) == "SELL"


def test_builder_stop_loss_exits_when_spread_moves_against_selected_direction():
    strategy = _build_builder_strategy(request(cash_side="BUY", future_side="SELL", stop_loss=2.0))
    assert strategy(point(5.0), ()) == "BUY"
    assert strategy(point(7.5), (point(5.0),)) == "SELL"


def test_runner_default_direction_applies_gap_profit_and_capital():
    req = request(cash_side="BUY", future_side="SELL", target=2.0)
    strategy = _build_builder_strategy(req)
    result = run_cash_future_strategy([point(5.0), point(3.0, minute=16)], strategy, strategy_id=req.strategy_id, config=CashFutureStrategyConfig(initial_capital=10_000.0))
...
def test_runner_reverse_direction_applies_opposite_gap_profit():
    req = request(cash_side="SELL", future_side="BUY", target=2.0)
    strategy = _build_builder_strategy(req)
    result = run_cash_future_strategy([point(-5.0), point(-3.0, minute=16)], strategy, strategy_id=req.strategy_id, config=CashFutureStrategyConfig(initial_capital=10_000.0, cash_side="SELL", future_side="BUY"))
    assert len(result.trades) == 1
    assert result.trades[0]["gross_profit"] == 20.0
    assert result.net_profit == 20.0
...
def test_runner_slippage_is_deducted_for_both_legs():
    req = request(cash_side="BUY", future_side="SELL", target=2.0)
    strategy = _build_builder_strategy(req)
    result = run_cash_future_strategy([point(5.0), point(3.0, minute=16)], strategy, strategy_id=req.strategy_id, config=CashFutureStrategyConfig(initial_capital=10_000.0, slippage_per_share=0.5))
    assert len(result.trades) == 1
