...
def test_full_fno_job_is_queued_and_persists_future_selection():
    ...
    original_submit = backtest_jobs._EXECUTOR.submit
    backtest_jobs._EXECUTOR.submit = fake_submit
    try:
        ...
        try:
            ...
            assert loaded is not None and loaded.symbol == "__FULL_FNO__" and loaded.contract_month == "BOTH" and loaded.status == "queued"
            assert submitted["args"][0] == job.job_id
        finally:
            db.query(BacktestJobResultChunk).filter(BacktestJobResultChunk.job_id == job.job_id).delete(); db.query(BacktestJob).filter(BacktestJob.job_id == job.job_id).delete(); db.commit(); db.close()
    finally:
        backtest_jobs._EXECUTOR.submit = original_submit


def test_full_fno_job_can_be_cancelled_deterministically(monkeypatch):
    """Cancellation must be testable while the worker is still queued, without an executor race."""
    Base.metadata.create_all(bind=engine)
    class QueuedFuture:
        def running(self): return False
        def cancel(self): return True
    monkeypatch.setattr(backtest_jobs._EXECUTOR, "submit", lambda fn, *args: QueuedFuture())
    ...
    try:
        ...
        loaded = backtest_jobs.get_job(db, job.job_id)
        assert loaded is not None and loaded.status == "cancelled" and loaded.contract_month == "CURRENT"
    finally:
        db.query(BacktestJobResultChunk).filter(BacktestJobResultChunk.job_id == job.job_id).delete(); db.query(BacktestJob).filter(BacktestJob.job_id == job.job_id).delete(); db.commit(); db.close()
...
def test_full_fno_result_chunks_are_durable_and_bounded():
    ...
    db = SessionLocal()
    try:
        db.add(job); db.commit(); backtest_jobs._persist_full_fno_chunk(job.job_id, 0, "RELIANCE", {"net_profit": 125.0}); backtest_jobs._persist_full_fno_chunk(job.job_id, 1, "TCS", {"net_profit": -25.0})
        ...
        assert len(chunks) == 1 and chunks[0].sequence == 0 and chunks[0].symbol == "RELIANCE" and chunks[0].result_json == '{"net_profit": 125.0}'
        db.expire_all(); loaded = db.query(BacktestJobResultChunk).filter(BacktestJobResultChunk.job_id == job.job_id, BacktestJobResultChunk.sequence == 1).one()
        assert loaded.symbol == "TCS" and loaded.result_json == '{"net_profit": -25.0}'
    finally:
        db.query(BacktestJobResultChunk).filter(BacktestJobResultChunk.job_id == job.job_id).delete(); db.query(BacktestJob).filter(BacktestJob.job_id == job.job_id).delete(); db.commit(); db.close()
...
def test_full_fno_incremental_mode_sinks_each_symbol_without_result_list(monkeypatch):
    ...
    def fake_engine(bars, config, cancelled=None):
        calls.append((next(iter(bars)), config.collect_ledger, config.future_selection)); return {"status": "completed", "net_profit": 100.0, "max_drawdown": 0.0}
    ...
    assert result["status"] == "completed" and result["results"] is None and result["symbols_total"] == 3 and result["symbols_processed"] == 3 and result["chunks_written"] == 3
    assert [symbol for _, symbol, _ in sink_calls] == ["RELIANCE", "TCS", "INFY"] and [sequence for sequence, _, _ in sink_calls] == [0, 1, 2]
    assert all(collect_ledger is False for _, collect_ledger, _ in calls) and all(selection == "BOTH" for _, _, selection in calls) and result["total_net_profit"] == 300.0
...
def test_paper_engine_does_not_retain_minute_ledger_when_incremental():
    ...
    assert "ledger" not in result and result["status"] == "completed"
...
def test_full_fno_completed_job_keeps_only_compact_summary_and_reopens_chunks(monkeypatch):
    ...
    try:
        ...
        backtest_jobs._run_full_fno_job(job.job_id, 365, 5.0, 0.0, 10.0, 2.0, 30, "BOTH"); db.expire_all(); loaded = backtest_jobs.get_job(db, job.job_id)
        assert loaded is not None and loaded.status == "completed" and loaded.symbols_processed == 2 and loaded.symbols_total == 2
        summary = json.loads(loaded.result_json); assert summary["future_selection"] == "BOTH" and summary["total_net_profit"] == 100.0 and "results" not in summary
    finally:
        db.query(BacktestJobResultChunk).filter(BacktestJobResultChunk.job_id == job.job_id).delete(); db.query(BacktestJob).filter(BacktestJob.job_id == job.job_id).delete(); db.commit(); db.close()
...
def test_full_fno_cancellation_stops_result_sinking(monkeypatch):
    ...
    monkeypatch.setattr(full_fno_backtest, "persisted_stock_symbols", lambda db: ["RELIANCE", "TCS", "INFY"])
    monkeypatch.setattr(full_fno_backtest, "iter_persisted_symbol_replay", lambda db, symbol, start, end: iter([symbol]))
    sink_calls = []; cancel_checks = {"count": 0}
    def cancelled():
        cancel_checks["count"] += 1; return cancel_checks["count"] >= 3
    def fake_engine(bars, config, cancelled=None):
        symbol = next(iter(bars)); return {"status": "cancelled", "net_profit": 0.0, "max_drawdown": 0.0, "symbol": symbol}
    monkeypatch.setattr(full_fno_backtest, "run_cash_future_paper_backtest", fake_engine)
