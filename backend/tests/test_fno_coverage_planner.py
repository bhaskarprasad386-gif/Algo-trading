...
def make_plan(catalog, calendar, session, interval, *, max_request_ns=None, end_ns=None):
    return build_fno_coverage_plan(
        make_universe(),
        as_of=date(2026, 9, 7),
        timeframe="1m",
        start_ns=session.start_ns,
        end_ns=session.end_ns if end_ns is None else end_ns,
...
def test_coverage_plan_uses_bounded_chunks_for_missing_session_edges():
    ...
    plan = make_plan(catalog, calendar, session, interval, max_request_ns=2, end_ns=session.start_ns + 4)
