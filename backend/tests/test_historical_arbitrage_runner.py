    ...
    def entry(event):
        if event["timestamp_ns"] == 1:
            return [OpenPosition(
                ...
                timestamp_ns=1,
                instrument="NFO:ABC",
                side="LONG_NEAR_SHORT_FAR",
                quantity=1,
                entry_price=100,
                contract="NEAR:2026-09-24|FAR:2026-10-29",
                expiry="2026-09-24/2026-10-29",
                strike=25000,
                leg="NEAR_FAR",
                data_resolution="s",
            )]
    ...
    def exit(position, event):
        if event["timestamp_ns"] == 3:
            return ExitExecution(
                ...
                fees=1,
                slippage=1,
                metadata={"exit_source": "real_quote"},
            )
        return None

    assert runner.replay(
        [{"timestamp_ns": 1}, {"timestamp_ns": 2}, {"timestamp_ns": 3}, {"timestamp_ns": 4}],
        entry,
        exit,
    ) == 1
...
def test_multiple_unresolved_positions_get_distinct_audit_sequences() -> None:
    ...
    def entry(event):
        if event["timestamp_ns"] != 1:
            return []
        return [
            OpenPosition("p1", 1, "NFO:A", "LONG", 1, 10),
            OpenPosition("p2", 1, "NFO:B", "LONG", 1, 20),
        ]
