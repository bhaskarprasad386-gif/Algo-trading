...
def test_cash_future_calculates_gap_profit_and_roi():
    ...
    future = FutureQuote("RELIANCE", "current", 1010.0, 250, 50000.0, 10000, 20000, 1010.0, 1011.0)
...
def test_custom_filters_reject_weak_opportunity():
    ...
    future = FutureQuote("ABC", "near", 100.2, 50, 10000.0, 100, 1000, 100.1, 100.3, date.today() + timedelta(days=2))
...
def test_bad_prices_are_rejected():
    with pytest.raises(ValueError):
        calculate_cash_future(CashQuote(symbol="ABC", ltp=0), FutureQuote("ABC", "current", 101, 10, 1000), CashFutureConfig())
...
def test_executable_gap_uses_cash_ask_and_future_bid():
    result = calculate_cash_future(
        ...
        FutureQuote("ABC", "current", 102.0, 10, 1000, bid=101.5, ask=102.5),
        CashFutureConfig(),
    )
...
def test_execution_filters_reject_missing_quotes_and_wide_spreads():
    missing = calculate_cash_future(
        ...
        FutureQuote("ABC", "current", 102.0, 10, 1000, bid=101.0, ask=103.0),
    ...
    wide = calculate_cash_future(
        ...
        FutureQuote("ABC", "current", 102.0, 10, 1000, bid=100.0, ask=105.0),
...
def test_malformed_quote_direction_is_rejected():
    ...
    future = calculate_cash_future(
        CashQuote(symbol="ABC", ltp=100.0, bid=99.0, ask=100.0),
        FutureQuote("ABC", "current", 102.0, 10, 1000, bid=103.0, ask=102.0),
        CashFutureConfig(),
    )
...
def test_non_positive_executable_gap_is_never_executable():
    result = calculate_cash_future(
        ...
        FutureQuote("ABC", "current", 102.0, 10, 1000, bid=100.5, ask=102.0),
        CashFutureConfig(),
    )
...
def test_non_positive_quote_sides_are_rejected():
    cash = calculate_cash_future(
        ...
        FutureQuote("ABC", "current", 102.0, 10, 1000, bid=101.0, ask=102.0),
        CashFutureConfig(),
    )
    future = calculate_cash_future(
        CashQuote(symbol="ABC", ltp=100.0, bid=99.0, ask=100.0),
        FutureQuote("ABC", "current", 102.0, 10, 1000, bid=0.0, ask=102.0),
        CashFutureConfig(),
    )
    assert cash.executable is False
    assert "invalid_cash_bid_ask" in cash.rejection_reasons
...
def test_one_sided_non_positive_quotes_are_rejected():
    cases = [
        (CashQuote(symbol="ABC", ltp=100.0, bid=0.0, ask=None), FutureQuote("ABC", "current", 102.0, 10, 1000, bid=101.0, ask=102.0), "invalid_cash_bid_ask"),
        (CashQuote(symbol="ABC", ltp=100.0, bid=None, ask=0.0), FutureQuote("ABC", "current", 102.0, 10, 1000, bid=101.0, ask=102.0), "invalid_cash_bid_ask"),
        (CashQuote(symbol="ABC", ltp=100.0, bid=99.0, ask=100.0), FutureQuote("ABC", "current", 102.0, 10, 1000, bid=0.0, ask=None), "invalid_future_bid_ask"),
        (CashQuote(symbol="ABC", ltp=100.0, bid=99.0, ask=100.0), FutureQuote("ABC", "current", 102.0, 10, 1000, bid=None, ask=0.0), "invalid_future_bid_ask"),
    ]
...
def test_profit_and_roi_thresholds_are_part_of_executable_decision():
    result = calculate_cash_future(
        CashQuote(symbol="ABC", ltp=100.0, bid=99.0, ask=100.0),
        FutureQuote("ABC", "current", 101.0, 10, 10000, bid=100.5, ask=101.0),
