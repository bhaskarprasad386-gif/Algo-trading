...
def test_full_fno_job_queues_and_persists_job_state(monkeypatch):
    ...
    job = backtest_jobs.create_full_fno_job(
        days=365, min_entry_gap=5.0, exit_gap=0.0, charges_per_trade=10.0,
        funding_cost_per_trade=2.0, max_holding_days=30,
    )
    db = SessionLocal()
    try:
    ...
    try:
        ...
        db.add_all([
            BacktestJobResultChunk(
                job_id=job_id, sequence=sequence, symbol=f"SYM{sequence}",
                result_json=f'{{"sequence": {sequence}}}', created_at=datetime.utcnow(),
            )
...
def test_full_fno_worker_failure_preserves_already_committed_chunks(monkeypatch):
    ...
    try:
        ...
        db.commit()
        db.add(BacktestJob(
            job_id=job_id, status="queued", symbol="__FULL_FNO__", contract_month="BOTH",
            requested_days=365, progress_pct=0.0, symbols_processed=0, symbols_total=1,
...
def test_full_fno_cancellation_does_not_persist_current_symbol_result(monkeypatch):
    """Cancellation during replay must leave only previously committed chunks durable."""
    Base.metadata.create_all(bind=engine)
    job_id = "test-job-cancel-during-replay"
    db = SessionLocal()
    try:
        db.query(BacktestJobResultChunk).filter(BacktestJobResultChunk.job_id == job_id).delete()
        db.query(BacktestJob).filter(BacktestJob.job_id == job_id).delete()
        ...
        db.add(BacktestJob(
            job_id=job_id, status="queued", symbol="__FULL_FNO__", contract_month="BOTH",
            requested_days=365, progress_pct=0.0, symbols_processed=0, symbols_total=2,
            message="Queued", created_at=datetime.utcnow(), updated_at=datetime.utcnow(),
        ))
        db.commit()
    finally:
    ...
    def fake_run(db, **kwargs):
        ...
        kwargs["cancelled"]()
        return {"status": "cancelled", "net_profit": 50.0}
