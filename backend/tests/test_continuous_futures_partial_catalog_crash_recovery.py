...
def test_running_chunk_crash_with_partial_catalog_data_resumes_only_missing_tail(tmp_path):
    ...
    # The worker dies while chunk 1 is running after several rows were committed.
    job_store.start_chunk("partial-crash-job", 1)
    partial = records[plan.requests[1]][:3]
    for record in partial:
        catalog.ingest(record)
    assert job_store.chunk_state("partial-crash-job", 1)[0] == "running"
    assert catalog.count(source="angelone", instrument="NFO:202", timeframe="1m") == 3

    catalog.close()
    job_store.close()

    # A new process must recover the running chunk and preserve its committed rows.
    catalog = HistoricalCatalog(catalog_path)
    job_store = HistoricalJobStore(job_store_path)
    assert job_store.chunk_state("partial-crash-job", 1)[0] == "running"

    second_source = RecordingSource(records)
    ...
    resumed_request = second_source.calls[0]
    original_request = plan.requests[1]
    assert resumed_request.start_ns == original_request.start_ns + 3 * interval_ns
    assert resumed_request.end_ns == original_request.end_ns
    assert job_store.chunk_state("partial-crash-job", 1)[0] == "completed"
    assert job_store.pending_indices("partial-crash-job") == ()
    assert catalog.count(source="angelone", instrument="NFO:101", timeframe="1m") == len(records[plan.requests[0]])
