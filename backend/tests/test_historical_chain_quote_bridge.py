...
from app.backtesting.historical_chain_quote_bridge import future_quote, option_quote, option_quotes
...
def test_option_quotes_skip_incomplete_strikes_without_fabrication():
    quotes = option_quotes((
        _contract(strike=100, option_type="CE"),
        _contract(strike=100, option_type="PE"),
        _contract(strike=110, option_type="CE"),
    ))
    assert len(quotes) == 1
...
def test_future_quote_requires_genuine_liquidity_fields():
    quote = future_quote({
        "timestamp_ns": 1, "underlying": "ABC", "expiry": 20260924,
        "bid": 120, "ask": 121, "lot_size": 50, "instrument_class": "INDEX",
