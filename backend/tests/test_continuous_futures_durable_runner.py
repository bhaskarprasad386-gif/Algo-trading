...
from app.backtesting.continuous_futures_durable_runner import (
    run_continuous_futures_events_to_durable_ledger,
)
...
def test_continuous_futures_durable_runner_saves_summary(tmp_path):
    windows = (
    ...
    with BacktestTradeLedger(tmp_path / "ledger.sqlite") as ledger:
        result = run_continuous_futures_events_to_durable_ledger(
            ...
            strategy,
            ledger=ledger,
            run_id="cash-future-rollover",
            chunk_size=1,
        )

        summary = ledger.run_summary("cash-future-rollover")
        assert summary is not None
        assert summary["final_capital"] == result.final_capital
        assert summary["net_pnl"] == result.net_pnl
        assert ledger.count("cash-future-rollover") == 1
        assert ledger.net_pnl("cash-future-rollover") == 8.0
