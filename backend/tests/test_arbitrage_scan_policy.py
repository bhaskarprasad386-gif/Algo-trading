...
def test_stock_box_uses_strike_positions_not_rupee_gap():
...
def test_synthetic_stock_is_both_sides_of_atm_with_max_five_positions():
    result = enumerate_synthetic_strikes(
        STRIKES, atm_strike=100, instrument_class="STOCK"
    )
...
def test_synthetic_index_allows_fifteen_positions_when_chain_has_them():
    ...
    result = enumerate_synthetic_strikes(
        strikes, atm_strike=150, instrument_class="INDEX"
    )
