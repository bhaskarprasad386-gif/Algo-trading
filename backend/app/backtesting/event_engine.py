...
class EventBacktestEngine:
    ...
    def _try_execute_orders(self, orders: Iterable[SimOrder], event: MarketEvent) -> tuple[SimFill, ...]:
        ...
        for order in orders:
            ...
            if order.order_type.name == "STOP" and (observed_price is None or not stop_triggered(order, observed_price)): continue
            reference_price = self._order_reference_price(order, observed)
            if reference_price is None: continue
            candidates.append((order, observed, reference_price)); lifecycles[order.order_id] = lifecycle
        ...
        for order, _, _ in candidates:
            ...
            if not lifecycle.state.terminal and result is not None:
                if order.time_in_force == TimeInForce.IOC:
                    lifecycle.cancel(event.timestamp_ns, result.reason or "IOC residual cancelled")
                elif order.time_in_force == TimeInForce.FOK and lifecycle.state.status in {OrderStatus.SUBMITTED, OrderStatus.ACCEPTED}:
                    lifecycle.reject(result.reason or "FOK residual rejected", event.timestamp_ns)
            ...
            reserved = self._reserved_margin.get(order.order_id, 0.0)
            if filled_qty and reserved:
                release = reserved * filled_qty / order.quantity
                self.portfolio.release_margin(order.order_id, release)
                self._reserved_margin[order.order_id] = max(0.0, reserved - release)
            if lifecycle.state.terminal:
                if self._reserved_margin.get(order.order_id, 0.0):
    ...
    def _execute_decision(self, decision: StrategyDecision, event: MarketEvent) -> tuple[SimFill, ...]:
        ...
        try:
            for raw_order in decision.orders:
                ...
                order = self._submit_effective_order(raw_order, event)
                lifecycle = self._lifecycle(order, order.submitted_at_ns)
                if lifecycle.state.terminal: continue
                if order.order_id not in self._reserved_margin:
                    observed = self._latest_events.get(order.instrument)
                    reference = self._order_reference_price(order, observed) if observed is not None else None
            ...
            self._risk_blocks += 1
            for order_id in list(self._reserved_margin):
                if order_id in {o.order_id for o in effective_orders}: self.portfolio.release_margin(order_id); self._reserved_margin.pop(order_id, None); self._open_orders.pop(order_id, None)
            return ()
    ...
    def market_state(self) -> Mapping[str, object]:
        return {"latest_events": [
            {"timestamp_ns": e.timestamp_ns, "instrument": e.instrument, "event_type": e.event_type.value,
             "payload": dict(e.payload), "sequence": e.sequence, "source": e.source} for e in self._latest_events.values()
        ], "latest_books": [
            ...
            {"order_id": o.order_id, "instrument": o.instrument, "side": o.side.value, "quantity": o.quantity,
             ...
             "time_in_force": o.time_in_force.value} for o in self._open_orders.values()
    ...
    def restore_market_state(self, state: Mapping[str, object]) -> None:
        ...
        for raw in state.get("latest_events", []):
            e = MarketEvent(int(raw["timestamp_ns"]), str(raw["instrument"]), EventType(raw["event_type"]), dict(raw.get("payload", {})), raw.get("sequence"), raw.get("source"))
            self._latest_events[e.instrument] = e
        for raw in state.get("latest_books", []):
            book = OrderBook(bids=tuple(DepthLevel(float(p), int(q)) for p, q in raw.get("bids", [])), asks=tuple(DepthLevel(float(p), int(q)) for p, q in raw.get("asks", [])))
            self._latest_books[str(raw["instrument"])] = (int(raw["timestamp_ns"]), book)
        for raw in state.get("open_orders", []):
            order = SimOrder(order_id=str(raw["order_id"]), instrument=str(raw["instrument"]), side=ExecutionSide(raw["side"]), quantity=int(raw["quantity"]),
                order_type=__import__("app.backtesting.execution", fromlist=["OrderType"]).OrderType(raw["order_type"]), limit_price=raw.get("limit_price"), stop_price=raw.get("stop_price"),
    ...
    def run(self, events: Iterable[MarketEvent], strategy: object, *, state: Mapping[str, object] | None = None,
            start_event_index: int = 0, checkpoint_callback: Callable[[int, int, Mapping[str, object]], None] | None = None,
        ...
        if start_event_index == 0:
            self._latest_events.clear(); self._latest_books.clear(); self._order_lifecycles.clear(); self._open_orders.clear(); self._reserved_margin.clear()
        for raw_index, raw_event in enumerate(events):
            ...
            retry_fills = self._try_execute_orders(tuple(self._open_orders.values()), event)
            fill_count += len(retry_fills)
