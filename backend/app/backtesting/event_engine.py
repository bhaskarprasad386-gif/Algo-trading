...
class EventBacktestEngine:
    ...
    def _market_risk_allows_order(self, order: SimOrder) -> bool:
        ...
        try:
            ...
            return order_reduces_position_risk(self.portfolio, order, self._current_marks())
    ...
    def replace_order(self, order_id: str, replacement: SimOrder, timestamp_ns: int, queue_ahead_quantity: int | None = None) -> SimOrder:
        ...
        if self.portfolio is not None and not order_reduces_position_risk(self.portfolio, effective, self._current_marks()):
    ...
    def _execute_decision(self, decision: StrategyDecision, event: MarketEvent) -> tuple[SimFill, ...]:
        ...
        try:
            for raw_order in decision.orders:
                ...
                risk_reducing = order_reduces_position_risk(self.portfolio, order, self._current_marks())
                if not self._market_risk_allows_order(order):
