...
class EventBacktestEngine:
    ...
    def _lifecycle(self, order: SimOrder, timestamp_ns: int) -> OrderLifecycle:
        ...
        if lifecycle is None:
            ...
            self._journal_lifecycle(lifecycle.state.events[-1])
    ...
    def cancel_order(self, order_id: str, timestamp_ns: int, reason: str = "cancelled") -> None:
        ...
        lifecycle.cancel(timestamp_ns, reason); self._journal_lifecycle(lifecycle.state.events[-1])
        state = self._queue_lifecycles.get(order_id)
    ...
    def replace_order(self, order_id: str, replacement: SimOrder, timestamp_ns: int, queue_ahead_quantity: int | None = None) -> SimOrder:
        ...
        if self.portfolio is not None and not order_reduces_position_risk(self.portfolio, effective):
            ...
            if self.portfolio is not None: self.portfolio.release_margin(order_id)
        lifecycle.replace(effective, timestamp_ns); self._journal_lifecycle(lifecycle.state.events[-1])
    ...
    def _try_execute_orders(self, orders: Iterable[SimOrder], event: MarketEvent) -> tuple[SimFill, ...]:
        if self.execution is None or self.portfolio is None: return ()
        ...
        for order in orders:
            ...
            if not self._market_risk_allows_order(order):
                self._risk_blocks += 1; lifecycle.reject("market risk violation", event.timestamp_ns); self._journal_lifecycle(lifecycle.state.events[-1]); self.portfolio.release_margin(order.order_id)
            ...
            if order.order_type == OrderType.STOP and (observed_price is None or not stop_triggered(order, observed_price)): continue
            reference_price = self._order_reference_price(order, observed)
            if reference_price is None: continue
            candidates.append((self._execution_order(order), observed, reference_price)); lifecycles[order.order_id] = lifecycle
        fills = []; results = {}
        for order, observed, _ in candidates:
            ...
            if book_state is not None and book_state[0] == observed.timestamp_ns:
                ...
                if order.time_in_force == TimeInForce.FOK and result.remaining_quantity > 0:
                    ...
                    if lifecycle.state.status in {OrderStatus.SUBMITTED, OrderStatus.ACCEPTED}: lifecycle.reject(result.reason or "FOK not fully executable", event.timestamp_ns); self._journal_lifecycle(lifecycle.state.events[-1])
                ...
                if price is not None:
                    ...
                    if order.order_type == OrderType.LIMIT:
                        if order.limit_price is None: executable = False; reason = "limit_price is required"
                        elif order.side == ExecutionSide.BUY and price > order.limit_price: executable = False; reason = "limit price not executable"
                        elif order.side == ExecutionSide.SELL and price < order.limit_price: executable = False; reason = "limit price not executable"
                    if executable:
                        fills.append(self.execution.execute(order, price, event.timestamp_ns)); results[order.order_id] = type("ExecutionResult", (), {"remaining_quantity": 0, "rejected": False, "reason": None})()
                    else: results[order.order_id] = type("ExecutionResult", (), {"remaining_quantity": order.quantity, "rejected": False, "reason": reason})()
        try:
            if fills: self.portfolio.apply_fills_atomic(fills, self._current_marks())
        except RiskViolation:
            self._risk_blocks += 1
            for order, _, _ in candidates:
                lifecycle = lifecycles[order.order_id]
                if not lifecycle.state.terminal: lifecycle.reject("atomic execution risk violation", event.timestamp_ns); self._journal_lifecycle(lifecycle.state.events[-1])
        ...
        for order, _, _ in candidates:
            ...
            if not lifecycle.state.terminal and result is not None:
                ...
                elif order.time_in_force == TimeInForce.FOK and lifecycle.state.status in {OrderStatus.SUBMITTED, OrderStatus.ACCEPTED}: lifecycle.reject(result.reason or "FOK residual rejected", event.timestamp_ns); self._journal_lifecycle(lifecycle.state.events[-1])
            remaining = lifecycle.state.remaining_quantity; terminal_action = tif_after_execution(order.time_in_force, remaining)
            if terminal_action == OrderStatus.CANCELLED and not lifecycle.state.terminal: lifecycle.cancel(event.timestamp_ns, "IOC residual cancelled"); self._journal_lifecycle(lifecycle.state.events[-1])
            elif terminal_action == OrderStatus.REJECTED and not lifecycle.state.terminal: lifecycle.reject("FOK residual rejected", event.timestamp_ns); self._journal_lifecycle(lifecycle.state.events[-1])
            ...
            if lifecycle.state.terminal:
                ...
                self._open_orders[order.order_id] = order; state = self._queue_lifecycles.get(order.order_id, QueueLifecycleState(order.queue_ahead_quantity)); self._queue_lifecycles[order.order_id] = state; self._dynamic_queue_ahead[order.order_id] = state.queue_ahead_quantity
        return tuple(fills)
    ...
    def _execute_decision(self, decision: StrategyDecision, event: MarketEvent) -> tuple[SimFill, ...]:
        ...
        try:
            for raw_order in decision.orders:
                ...
                if not self._market_risk_allows_order(order):
                    self._risk_blocks += 1; lifecycle.reject("market risk violation", event.timestamp_ns); continue
                if order.order_id not in self._reserved_margin and not risk_reducing:
    ...
    def run(self, events: Iterable[MarketEvent], strategy: object, *, state: Mapping[str, object] | None = None, start_event_index: int = 0, checkpoint_callback: Callable[[int, int, Mapping[str, object]], None] | None = None, checkpoint_interval: int | None = None) -> ReplayStats:
        ...
        if start_event_index == 0: self._latest_events.clear(); self._latest_books.clear(); self._order_lifecycles.clear(); self._open_orders.clear(); self._reserved_margin.clear(); self._dynamic_queue_ahead.clear(); self._queue_lifecycles.clear()
        for raw_index, raw_event in enumerate(events):
            ...
            if checkpoint_callback is not None and checkpoint_interval and dispatched % checkpoint_interval == 0: checkpoint_callback(raw_index + 1, dispatched, {"context_state": dict(context_state), "market_state": self.market_state()})
        if started:
