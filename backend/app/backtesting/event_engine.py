...
class EventBacktestEngine:
    ...
    def _try_execute_orders(self, orders: Iterable[SimOrder], event: MarketEvent) -> tuple[SimFill, ...]:
        ...
        for order, observed, _ in candidates:
            ...
            if book_state is not None and book_state[0] == observed.timestamp_ns:
                ...
                if price is not None:
                    ...
                    results[order.order_id] = type("ExecutionResult", (), {
                        "remaining_quantity": 0, "rejected": False, "reason": None
                    })()
        ...
        try:
            ...
            for order, _, _ in candidates:
                ...
                if not lifecycle.state.terminal:
                    lifecycle.reject("atomic execution risk violation", event.timestamp_ns)
        ...
        for order, _, _ in candidates:
            lifecycle = lifecycles[order.order_id]
            for fill in (f for f in fills if f.order_id == order.order_id):
    ...
    def _execute_decision(self, decision: StrategyDecision, event: MarketEvent) -> tuple[SimFill, ...]:
        ...
        try:
            for raw_order in decision.orders:
                if not isinstance(raw_order, SimOrder):
        ...
        return {
            ...
            "latest_books": [
                {"instrument": instrument, "timestamp_ns": ts,
                 "bids": [(x.price, x.quantity) for x in book.bids],
                 "asks": [(x.price, x.quantity) for x in book.asks]}
    ...
    def restore_market_state(self, state: Mapping[str, object]) -> None:
        ...
        for raw in state.get("open_orders", []):
            order = SimOrder(order_id=str(raw["order_id"]), instrument=str(raw["instrument"]),
                side=ExecutionSide(raw["side"]), quantity=int(raw["quantity"]),
                order_type=OrderType(raw["order_type"]), limit_price=raw.get("limit_price"),
                stop_price=raw.get("stop_price"), submitted_at_ns=int(raw["submitted_at_ns"]),
    ...
    def run(self, events: Iterable[MarketEvent], strategy: object, *, state: Mapping[str, object] | None = None,
        ...
        if start_event_index < 0:
            raise ValueError("start_event_index cannot be negative")
        ...
        for raw_index, raw_event in enumerate(events):
            ...
            event = raw_event if timestamp_ns == raw_event.timestamp_ns else MarketEvent(
                timestamp_ns, raw_event.instrument, raw_event.event_type, raw_event.payload,
            ...
            fill_count += len(self._try_execute_orders(tuple(self._open_orders.values()), event))
            decision = None
            ...
            if checkpoint_callback is not None and checkpoint_interval and dispatched % checkpoint_interval == 0:
                checkpoint_callback(raw_index + 1, dispatched,
                                    {"context_state": dict(context_state), "market_state": self.market_state()})
        if started:
