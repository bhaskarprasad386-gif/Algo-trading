...
def test_partial_fill_preserves_source_backed_dynamic_queue():
    ...
    event = _depth(2, [{"price": 100.0, "executed_quantity": 3}], ask_qty=2)
    ...
    assert engine._dynamic_queue_ahead[order.order_id] == 0
...
def test_cancel_order_closes_current_queue_generation():
    ...
    engine._open_orders[order.order_id] = order
    engine._dynamic_queue_ahead[order.order_id] = 8
    engine.cancel_order(order.order_id, 2, "test")
    assert order.order_id not in engine._open_orders
    assert engine.queue_states[order.order_id].status == "CANCELLED"
