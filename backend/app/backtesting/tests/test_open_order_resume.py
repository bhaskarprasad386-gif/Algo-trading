...
def test_resume_restores_open_order_and_finishes_same_as_uninterrupted(tmp_path):
    ...
    assert resumed_portfolio.snapshot(marks).cash == pytest.approx(full_portfolio.snapshot(marks).cash)
    assert resumed_portfolio.snapshot(marks).positions == full_portfolio.snapshot(marks).positions
