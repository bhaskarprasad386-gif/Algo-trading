...
def _historical_fill_capacity(point: CashFutureHistoryPoint, *, side: str, requested_quantity: float, execution_model: str) -> tuple[float, str]:
    ...
    if side == "entry":
        prices = (point.cash_ask, point.future_bid); required = (point.cash_ask_qty, point.future_bid_qty)
    else:
        prices = (point.cash_bid, point.future_ask); required = (point.cash_bid_qty, point.future_ask_qty)
    ...
    return min(requested, min(numeric_depth)), "historical_depth"
...
def run_cash_future_portfolio_strategy(points: Iterable[CashFutureHistoryPoint], strategy: PortfolioStrategy, *, initial_capital: float = 100_000_000.0, execution_model: str = "gap", charges_per_trade: float = 0.0, funding_cost_per_trade: float = 0.0, rollover_policy: str = "force_exit") -> CashFuturePortfolioStrategyRun:
    ...
    if any((current.timestamp, current.symbol, current.contract_month) < (previous.timestamp, previous.symbol, previous.contract_month) for previous, current in zip(ordered, ordered[1:])):
        raise ValueError("Cash-Future portfolio input must be deterministically ordered by timestamp, symbol and contract")

    account = CashFuturePortfolioLedger(initial_capital)
    histories: dict[str, list[CashFutureHistoryPoint]] = {}
    ...
    signals: list[Mapping[str, Any]] = []; trades: list[Mapping[str, Any]] = []; equity: list[Mapping[str, Any]] = []
    ...
    for point in ordered:
        ...
        symbol_history = histories.setdefault(point.symbol, [])
        visible_history = tuple(symbol_history + [point])
        raw_signal = strategy(point, visible_history)
        symbol_history.append(point)
        latest[key] = point
        ...
        if action == "BUY" and entry_state is None:
            ...
            if expiry_day:
                signal_record.update({"execution_status": "blocked", "blocked_reason": "expiry_day_new_entry", "requested_quantity": requested_quantity, "filled_quantity": 0.0})
            else:
        ...
        elif action == "BUY" and entry_state is not None:
            account.blocked_entries += 1
            signal_record.update({"execution_status": "blocked", "blocked_reason": "position_already_open"})
        elif action == "SELL" and entry_state is None:
            signal_record.update({"execution_status": "rejected", "rejection_reason": "no_open_position"})
        else:
            signal_record["execution_status"] = "not_executed"
        ...
        unrealized = _marked_equity(account, entries, latest, execution_model) - float(account.realized_capital)
    ...
    final_unrealized = _marked_equity(account, entries, latest, execution_model) - float(account.realized_capital)
