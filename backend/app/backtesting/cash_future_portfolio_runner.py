...
def _validate_sides(cash_side:str,future_side:str)->None:
    if cash_side not in {"BUY","SELL"} or future_side not in {"BUY","SELL"} or cash_side==future_side: raise ValueError("cash_side and future_side must be opposite BUY/SELL sides")

def _unrealized_profit(entry,current,execution_model,quantity,cash_side="BUY",future_side="SELL"):
    direction=1.0 if (cash_side,future_side)==("BUY","SELL") else -1.0
    if execution_model=="gap": return direction*_legacy_gap_profit(entry,current)*(quantity/entry.lot_size)
    if (cash_side,future_side)==("BUY","SELL"): prices=(entry.cash_ask,entry.future_bid,current.cash_bid,current.future_ask)
    else: prices=(entry.cash_bid,entry.future_ask,current.cash_ask,current.future_bid)
    if any(price is None or not math.isfinite(float(price)) or float(price)<=0 for price in prices): return 0.0
    if (cash_side,future_side)==("BUY","SELL"):
        gross=(current.cash_bid-entry.cash_ask)+(entry.future_bid-current.future_ask)
    else:
        gross=(entry.cash_bid-current.cash_ask)+(current.future_ask-entry.future_bid)
    return gross*entry.lot_size*(quantity/entry.lot_size)

def _historical_fill_capacity(point,*,side,requested_quantity,execution_model,cash_side="BUY",future_side="SELL"):
    ...
    entry_quotes=(point.cash_ask,point.future_bid) if (cash_side,future_side)==("BUY","SELL") else (point.cash_bid,point.future_ask)
    exit_quotes=(point.cash_bid,point.future_ask) if (cash_side,future_side)==("BUY","SELL") else (point.cash_ask,point.future_bid)
    prices=entry_quotes if side=="entry" else exit_quotes
    if (cash_side,future_side)==("BUY","SELL"):
        required=(point.cash_ask_qty,point.future_bid_qty) if side=="entry" else (point.cash_bid_qty,point.future_ask_qty)
    else:
        required=(point.cash_bid_qty,point.future_ask_qty) if side=="entry" else (point.cash_ask_qty,point.future_bid_qty)
...
def _close_position(account,entries,key,point,*,execution_model,charges_per_trade,funding_cost_per_trade,exit_reason,cash_side="BUY",future_side="SELL"):
...
def _marked_equity(account,entries,latest,execution_model,cash_side="BUY",future_side="SELL"):
    return float(account.realized_capital)+sum(_unrealized_profit(entry,latest[key],execution_model,quantity,cash_side,future_side) for key,(entry,quantity) in entries.items() if key in latest)

def run_cash_future_portfolio_strategy(points:Iterable[CashFutureHistoryPoint],strategy:PortfolioStrategy,*,initial_capital:float=100_000_000.0,execution_model:str="gap",charges_per_trade:float=0.0,funding_cost_per_trade:float=0.0,rollover_policy:str="force_exit",history_window:int=1000,cash_side:str="BUY",future_side:str="SELL")->CashFuturePortfolioStrategyRun:
    ...
    _validate_sides(cash_side,future_side)
    ...
    entry_action=cash_side; exit_action=future_side
    for point in ordered:
        ...
        signal_record={"timestamp":point.timestamp.isoformat(),"symbol":point.symbol,"contract_month":point.contract_month,"action":action,"cash_side":cash_side,"future_side":future_side,"lot_size":point.lot_size,"margin_required":point.margin_required}
        if action==entry_action and entry_state is None:
            ...
            if expiry_day:signal_record.update({"execution_status":"blocked","blocked_reason":"expiry_day_new_entry","requested_quantity":requested_quantity,"filled_quantity":0.0})
                ...
                fill_quantity,liquidity_source=_historical_fill_capacity(point,side="entry",requested_quantity=requested_quantity,execution_model=execution_model,cash_side=cash_side,future_side=future_side)
        ...
        elif action==entry_action and entry_state is not None:account.blocked_entries+=1;signal_record.update({"execution_status":"blocked","blocked_reason":"position_already_open"})
        elif action==exit_action and entry_state is None:signal_record.update({"execution_status":"rejected","rejection_reason":"no_open_position"})
        else:signal_record["execution_status"]="not_executed"
        entry_state=entries.get(key);exit_reason="strategy" if action==exit_action and entry_state is not None else None
        if entry_state is not None and expiry_day:exit_reason="expiry"
        signals.append(signal_record)
        if exit_reason is not None and entry_state is not None:
            trade=_close_position(account,entries,key,point,execution_model=execution_model,charges_per_trade=charges_per_trade,funding_cost_per_trade=funding_cost_per_trade,exit_reason=exit_reason,cash_side=cash_side,future_side=future_side)
            if trade is None:
        ...
        if entries and marked_equity<account.reserved_margin:
            ...
            for liquidation_key in sorted(tuple(entries)):
                ...
                liquidation_trade=_close_position(account,entries,liquidation_key,liquidation_point,execution_model=execution_model,charges_per_trade=charges_per_trade,funding_cost_per_trade=funding_cost_per_trade,exit_reason="margin_breach",cash_side=cash_side,future_side=future_side)
        ...
        if entries and _marked_equity(account,entries,latest,execution_model,cash_side,future_side)<account.reserved_margin:raise ValueError("portfolio margin breach remains after liquidation")
        unrealized=_marked_equity(account,entries,latest,execution_model,cash_side,future_side)-float(account.realized_capital);equity.append({"timestamp":point.timestamp.isoformat(),"equity":float(account.realized_capital)+unrealized,"realized_capital":float(account.realized_capital),"unrealized_pnl":unrealized,"available_capital":account.available_capital,"reserved_margin":account.reserved_margin,"open_position_count":account.open_position_count})
    final_unrealized=_marked_equity(account,entries,latest,execution_model,cash_side,future_side)-float(account.realized_capital);final_equity=float(account.realized_capital)+final_unrealized
