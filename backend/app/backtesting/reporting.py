...
def _timestamp_ns(timestamp: str) -> int:
    ...
    return (
        (utc.toordinal() - datetime(1970, 1, 1, tzinfo=timezone.utc).toordinal()) * _DAY_NS
        + utc.hour * 3_600_000_000_000
        + utc.minute * 60_000_000_000
        + utc.second * 1_000_000_000
        + utc.microsecond * 1_000
    )


def _year_month_from_ns(timestamp_ns: int) -> tuple[int, int]:
    """Derive calendar year/month without float timestamp conversion."""
    days = timestamp_ns // _DAY_NS
    date_value = datetime(1970, 1, 1, tzinfo=timezone.utc).date().fromordinal(
        datetime(1970, 1, 1, tzinfo=timezone.utc).date().toordinal() + days
    )
...
def build_cash_future_report(initial_capital: float, trades: Iterable[Mapping[str, object]], equity_curve: Iterable[Mapping[str, object]] = ()) -> BacktestReport:
    ...
    for trade in trades:
        try:
            ...
            legacy_lot = int(trade["lot_size"])
            filled = float(trade.get("filled_quantity", trade.get("quantity", legacy_lot)))
        except (KeyError, TypeError, ValueError) as exc:
            raise ValueError("invalid Cash-Future trade record") from exc
        if legacy_lot <= 0 or not isfinite(filled) or filled <= 0 or not all(isfinite(v) for v in (gross, net, charges, funding)):
...
def build_report(initial_capital: float, trades: Iterable[BacktestTrade]) -> BacktestReport:
    ...
    peak = equity
    max_dd = max_dd_pct = turnover = 0.0
    wins = losses = 0
    ...
    for trade in trades:
        ...
        if pnl > 0:
            ...
            losses += 1
            losses_abs += -pnl
        year, month_number = _year_month_from_ns(trade.timestamp_ns)
        month = f"{year:04d}-{month_number:02d}"
        year_key = f"{year:04d}"
        monthly[month] = monthly.get(month, 0.0) + pnl
        yearly[year_key] = yearly.get(year_key, 0.0) + pnl
