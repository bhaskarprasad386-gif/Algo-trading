...
def _validate_rollover_windows(windows: tuple[FNORolloverWindow, ...]) -> None:
    if not windows:
        return
    first = windows[0]
    validate_futures_rollover_chain(windows, underlying=first.underlying, instrument_type=first.instrument_type)
...
def build_catalog_continuous_futures_acquisition_plan(
    contract_master: ContractMasterCatalog,
    *,
    start_date, end_date, source: str, timeframe: str, interval_ns: int,
    calendar: TradingCalendar, max_request_ns: int, underlying: str | None = None,
    instrument_type: str = "STOCK_FUTURE", exchange: str = "NFO", snapshot_date=None,
) -> tuple[tuple[FNORolloverWindow, ...], HistoricalSyncPlan]:
    """Build a Cash-Future acquisition plan directly from the contract-master catalog."""
    if underlying is None:
        windows = build_catalog_futures_rollover_windows_for_universe(
            contract_master, start_date=start_date, end_date=end_date,
            instrument_type=instrument_type, exchange=exchange, snapshot_date=snapshot_date,
        )
    else:
        windows = build_catalog_futures_rollover_windows(
            contract_master, underlying=underlying, start_date=start_date, end_date=end_date,
            instrument_type=instrument_type, exchange=exchange, snapshot_date=snapshot_date,
        )
    return windows, build_continuous_futures_acquisition_plan(
        windows, source=source, timeframe=timeframe, interval_ns=interval_ns,
