...
def build_continuous_futures_acquisition_plan(windows: tuple[FNORolloverWindow, ...] | list[FNORolloverWindow], *, source: str, timeframe: str, interval_ns: int, calendar: TradingCalendar, max_request_ns: int) -> HistoricalSyncPlan:
...
def build_catalog_continuous_futures_acquisition_plan(contract_master: ContractMasterCatalog, *, start_date, end_date, source: str, timeframe: str, interval_ns: int, calendar: TradingCalendar, max_request_ns: int, underlying: str | None = None, instrument_type: str = "STOCK_FUTURE", exchange: str = "NFO", snapshot_date=None) -> tuple[tuple[FNORolloverWindow, ...], HistoricalSyncPlan]:
    if underlying is None:
        windows = build_catalog_futures_rollover_windows_for_universe(contract_master, start_date=start_date, end_date=end_date, instrument_type=instrument_type, exchange=exchange, snapshot_date=snapshot_date)
    else:
        windows = build_catalog_futures_rollover_windows(contract_master, underlying=underlying, start_date=start_date, end_date=end_date, instrument_type=instrument_type, exchange=exchange, snapshot_date=snapshot_date)
    return windows, build_continuous_futures_acquisition_plan(windows, source=source, timeframe=timeframe, interval_ns=interval_ns, calendar=calendar, max_request_ns=max_request_ns)


def _missing_ranges(catalog: HistoricalCatalog, *, source: str, instrument: str, timeframe: str, session_start_ns: int, session_end_ns: int, interval_ns: int) -> tuple[tuple[int, int], ...]:
    ...
    for timestamp in expected:
        if timestamp not in present:
            if range_start is None: range_start = timestamp
            previous_missing = timestamp
        elif range_start is not None:
            ranges.append((range_start, previous_missing)); range_start = previous_missing = None
    if range_start is not None: ranges.append((range_start, previous_missing))
    return tuple(ranges)


def build_continuous_futures_gap_plan(catalog: HistoricalCatalog, windows: tuple[FNORolloverWindow, ...] | list[FNORolloverWindow], *, source: str, timeframe: str, interval_ns: int, calendar: TradingCalendar, max_request_ns: int) -> HistoricalSyncPlan:
    _validate_inputs(source=source, timeframe=timeframe, interval_ns=interval_ns, max_request_ns=max_request_ns)
    windows = tuple(windows); _validate_rollover_windows(windows)
    requests: list[HistoricalFetchRequest] = []
    for window in windows:
...
def _plan_from_metadata(metadata: tuple[dict[str, object], ...]) -> HistoricalSyncPlan:
    return HistoricalSyncPlan(tuple(HistoricalFetchRequest(source=str(item["source"]), instrument=str(item["instrument"]), timeframe=str(item["timeframe"]), start_ns=int(item["start_ns"]), end_ns=int(item["end_ns"])) for item in metadata))


class _MissingRangeSource:
    def __init__(self, catalog: HistoricalCatalog, source: HistoricalSource, interval_ns: int) -> None: self.catalog, self.source, self.interval_ns = catalog, source, interval_ns
    def fetch(self, request: HistoricalFetchRequest):
        for start_ns, end_ns in _missing_ranges(self.catalog, source=request.source, instrument=request.instrument, timeframe=request.timeframe, session_start_ns=request.start_ns, session_end_ns=request.end_ns, interval_ns=self.interval_ns):
...
def repair_continuous_futures_history_gaps(catalog: HistoricalCatalog, source: HistoricalSource, windows: tuple[FNORolloverWindow, ...] | list[FNORolloverWindow], *, source_name: str, timeframe: str, interval_ns: int, calendar: TradingCalendar, max_request_ns: int, executor: ResumableHistoricalExecutor | None = None, job_store: HistoricalJobStore | None = None, job_id: str | None = None, run_id: str | None = None) -> ContinuousFuturesAcquisitionReport:
    windows = tuple(windows)
    if job_store is not None and job_id and run_id:
        try: existing = job_store.get(job_id)
        except KeyError: existing = None
        if existing is not None:
            if existing.run_id != run_id: raise ValueError("existing historical job does not match run or plan")
        ...
        else: plan = build_continuous_futures_gap_plan(catalog, windows, source=source_name, timeframe=timeframe, interval_ns=interval_ns, calendar=calendar, max_request_ns=max_request_ns)
    else: plan = build_continuous_futures_gap_plan(catalog, windows, source=source_name, timeframe=timeframe, interval_ns=interval_ns, calendar=calendar, max_request_ns=max_request_ns)
    runner = _runner(catalog, source, interval_ns, executor); gap_aware_source = _MissingRangeSource(catalog, source, interval_ns); should_skip = lambda request: _complete(catalog, request, interval_ns); should_accept = lambda request, result: _complete(catalog, request, interval_ns)
    if job_store is None:
        if job_id is not None or run_id is not None: raise ValueError("job_id and run_id require job_store")
        execution = runner.run(gap_aware_source, plan, should_skip=should_skip, should_accept=should_accept)
    else:
...
__all__ = ["ContinuousFuturesAcquisitionReport", "build_continuous_futures_acquisition_plan", "build_catalog_continuous_futures_acquisition_plan", "build_continuous_futures_gap_plan", "acquire_continuous_futures_history", "repair_continuous_futures_history_gaps", "build_continuous_futures_series_from_catalog"]
