...
class ChainContract:
    ...
    def __post_init__(self) -> None:
        ...
        if self.strike < 0 or self.volume < 0 or self.oi < 0 or self.bid < 0 or self.ask < self.bid:
...
def _validate_same_snapshot(contracts: Sequence[ChainContract], *, timestamp_ns: int | None = None, underlying: str | None = None, expiry: int | None = None) -> None:
    if not contracts:
        return
    first = contracts[0]
    expected = (first.timestamp_ns if timestamp_ns is None else timestamp_ns, first.underlying if underlying is None else underlying, first.expiry if expiry is None else expiry, first.venue, first.instrument_class)
    for contract in contracts:
        if (contract.timestamp_ns, contract.underlying, contract.expiry, contract.venue, contract.instrument_class) != expected:
            raise ValueError("historical chain contracts must share timestamp, underlying, expiry, venue and instrument class")

def _ordered_strikes(contracts: Iterable[ChainContract], *, timestamp_ns: int, underlying: str, expiry: int, option_type: str) -> list[float]:
    return sorted({c.strike for c in contracts if c.timestamp_ns == timestamp_ns and c.underlying == underlying and c.expiry == expiry and c.option_type == option_type})
...
def _select_side_positions(contracts: Sequence[ChainContract], *, atm: float, positions_below: int, positions_above: int, exclude_between: int = 0) -> tuple[ChainContract,...]:
    ...
    _validate_same_snapshot(contracts)
    strikes=sorted({c.strike for c in contracts}); ai=_atm_index(strikes,atm)
...
def select_calendar_expiries(contracts: Sequence[ChainContract]) -> tuple[tuple[int,int],...]:
    if not contracts: return ()
    _validate_same_snapshot(contracts)
    expiries=sorted({c.expiry for c in contracts}); return tuple((near,far) for i,near in enumerate(expiries) for far in expiries[i+1:])
...
def pair_by_strike(contracts: Sequence[ChainContract], *, expiry: int, option_type: str) -> tuple[SelectedPair,...]:
    ...
    _validate_same_snapshot(filtered, expiry=expiry)
    by_strike={}
    for c in filtered:
        if c.strike in by_strike: raise ValueError(f"duplicate strike in historical chain: {c.strike}")
        by_strike[c.strike]=c
    strikes=sorted(by_strike)
    return tuple(SelectedPair(by_strike[lo],by_strike[hi],i,j) for i,lo in enumerate(strikes) for j,hi in enumerate(strikes) if i<j)
