...
def _event_order_key(record: HistoricalRecord) -> tuple[object, ...]:
    """Canonical deterministic order across multiple event streams."""
...
def run_events_incremental(
    ...
    *,
    ...
    seen_identities: set[tuple[str, str, str, int, int | None]] = set()
    ...
    for record in events:
        ...
        if open_trade is None and action == "BUY":
            ...
            open_trade = (record.timestamp_ns, entry_price)
        ...
        if open_trade is not None:
            equity += _calculate_unrealized_pnl(engine_config, open_trade[1], price)
