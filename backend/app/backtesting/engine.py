...
class BacktestConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if self.quantity_step is not None and (self.quantity_step <= 0 or not _is_multiple(self.quantity, self.quantity_step)):
        ...
        for candle in candles:
            ...
            if self.config.execution_timing == "close":
                if open_trade is None and entry_strategy.evaluate(context):
                    ...
                    trade = _build_trade(self.config, entry_timestamp, entry_price, timestamp, exit_price, entry_market_price=close, exit_market_price=close, entry_price_source="bar:close", exit_price_source="bar:close")
                ...
                if pending_action is not None:
                    ...
                    if pending_action == "BUY" and open_trade is None:
                        ...
                        trade = _build_trade(self.config, entry_timestamp, entry_price, timestamp, exit_price, entry_market_price=None, exit_market_price=open_price, exit_price_source="bar:open")
                        capital += trade.net_pnl
                        trades.append(trade)
                        open_trade = None
    ...
    def run_events(self, events: Iterable[HistoricalRecord], strategy: EventStrategy, *, price_field: str = "price") -> BacktestResult:
        ...
        for record in records:
            ...
            if open_trade is None and signal.action == "BUY":
                ...
                _ensure_cash_available(self.config, capital, execution_price)
                open_trade = (record.timestamp_ns, execution_price, price, source, identity)
                ...
                trade = _build_trade(self.config, entry_timestamp, entry_price, record.timestamp_ns, exit_price, entry_market_price=entry_market_price, exit_market_price=price, entry_price_source=entry_source, exit_price_source=source, entry_event_identity=entry_identity, exit_event_identity=identity)
                capital += trade.net_pnl
                trades.append(trade)
                open_trade = None
...
def _event_order_key(record: HistoricalRecord) -> tuple:
