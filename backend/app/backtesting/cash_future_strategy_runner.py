...
def _builder_gap_profit(entry: CashFutureHistoryPoint, exit_point: CashFutureHistoryPoint, config: CashFutureStrategyConfig) -> float:
    ...
    if config.slippage_per_share:
        gross -= config.slippage_per_share * entry.lot_size * 2.0
    return gross


def _trade_gross_profit(entry: CashFutureHistoryPoint, exit_point: CashFutureHistoryPoint, config: CashFutureStrategyConfig) -> float:
    if config.execution_model == "gap":
        return _builder_gap_profit(entry, exit_point, config)
    gross = _executable_spread_profit(entry, exit_point)
    if (config.cash_side, config.future_side) != ("BUY", "SELL"):
        gross = -gross
    if config.slippage_per_share:
        gross -= config.slippage_per_share * entry.lot_size * 2.0
    return gross
...
def run_cash_future_strategy(
    ...
    *,
    ...
    if ledger is not None:
        ...
        ledger.start_run(
            run_id, strategy_id, strategy_version, config.initial_capital,
            strategy_hash=strategy_hash, data_source_fingerprint=data_source_fingerprint,
            metadata={
                "domain": "cash_future", "execution_model": config.execution_model,
                "start_date": config.start_date.isoformat() if config.start_date else None,
                "end_date": config.end_date.isoformat() if config.end_date else None,
                "contract_month": config.contract_month, "history_window": config.history_window,
                "checkpoint_interval": config.checkpoint_interval, "cash_side": config.cash_side,
    ...
    if ledger is not None and last_point is not None and config.checkpoint_interval is not None:
        _write_checkpoint(ledger, run_id, event_index, last_point, selected_contract, capital_ledger, entry, strategy, strategy_id, strategy_version, strategy_hash, data_source_fingerprint)
    if ledger is not None:
...
def _maybe_checkpoint(ledger, config: CashFutureStrategyConfig, run_id: str | None, event_index: int, point: CashFutureHistoryPoint, selected_contract: str | None, capital_ledger: CashFutureCapitalLedger, entry: CashFutureHistoryPoint | None, strategy, strategy_id: str, strategy_version: str, strategy_hash: str | None, data_source_fingerprint: str | None) -> None:
    if ledger is None or config.checkpoint_interval is None or event_index % config.checkpoint_interval != 0:
        return
...
def _write_checkpoint(ledger, run_id: str | None, event_index: int, point: CashFutureHistoryPoint, selected_contract: str | None, capital_ledger: CashFutureCapitalLedger, entry: CashFutureHistoryPoint | None, strategy, strategy_id: str, strategy_version: str, strategy_hash: str | None, data_source_fingerprint: str | None) -> None:
    if ledger is None or run_id is None:
        return
    checkpoint = CashFutureStrategyCheckpoint(
        run_id=run_id, strategy_id=strategy_id, strategy_version=strategy_version,
        strategy_hash=strategy_hash, last_timestamp=point.timestamp.isoformat(), selected_contract=selected_contract,
        realized_capital=float(capital_ledger.realized_capital), reserved_margin=float(capital_ledger.reserved_margin),
        blocked_entries=int(capital_ledger.blocked_entries), open_entry=_serialize_entry(entry),
        source_fingerprint=data_source_fingerprint, strategy_state=_capture_strategy_state(strategy),
    )
    from app.backtesting.ledger import Checkpoint
    ledger.checkpoint(Checkpoint(run_id=run_id, event_index=event_index, timestamp_ns=int(point.timestamp.timestamp() * 1_000_000_000), state=json.loads(checkpoint.to_json())))
...
def _capture_strategy_state(strategy) -> Mapping[str, Any] | None:
    ...
    try:
        ...
        raise ValueError("Cash-Future strategy checkpoint_state() must return JSON-serializable data") from exc
    return dict(state)
