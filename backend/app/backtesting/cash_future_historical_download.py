...
def _clip_sessions(sessions: Iterable[SessionWindow], start_ns: int, end_ns: int) -> tuple[SessionWindow, ...]:
    ...
    for session in sessions:
        start = max(session.start_ns, start_ns)
        end = min(session.end_ns, end_ns)
        if start <= end:
            clipped.append(SessionWindow(start, end))
...
class CashFutureHistoricalDownloadService:
    ...
    def _plan_for_request(request) -> HistoricalSyncPlan:
        return build_chunked_plan(source=request.source, instrument=request.instrument,
                                  timeframe=request.timeframe, start_ns=request.start_ns,
                                  end_ns=request.end_ns, chunk_ns=7 * 24 * 60 * 60 * 1_000_000_000)
    ...
    def _refresh_job_progress(self, job_id: str) -> None:
        ...
        self.status_store.update_job(job_id, completed_chunks=completed, skipped_chunks=skipped,
                                     failed_chunks=failed, catalog_count=self.catalog.count(),
                                     fetched_records=fetched, inserted_records=inserted)

    def _callbacks(self, job_id: str, sequence_offset: int = 0,
                   sequence_numbers: tuple[int, ...] | None = None):
        ...
        def start(index, request, attempt):
            persist_chunk_start(store, job_id=job_id, sequence=status_sequence(index), instrument=request.instrument,
        ...
        def skip(index, request):
            ...
            persist_chunk_result(store, job_id=job_id, sequence=status_sequence(index), instrument=request.instrument,
                                 ...
                                 first_missing_ns=first_missing, catalog_count=self.catalog.count())
            self._refresh_job_progress(job_id)
        ...
        def complete(index, request, result, attempt):
            ...
            persist_chunk_result(store, job_id=job_id, sequence=status_sequence(index), instrument=request.instrument,
                                 ...
                                 inserted_records=result.inserted, catalog_count=self.catalog.count())
            self._refresh_job_progress(job_id)
        ...
        def failed(index, request, error, attempts):
            ...
            persist_chunk_result(store, job_id=job_id, sequence=status_sequence(index), instrument=request.instrument,
                                 ...
                                 expected_timestamps=expected, actual_timestamps=actual, missing_timestamps=missing,
                                 first_missing_ns=first_missing, error=str(error), catalog_count=self.catalog.count())
            self._refresh_job_progress(job_id)
            store.update_job(job_id, status="FAILED", error=str(error))

        return {"on_chunk_start": start, "on_chunk_skip": skip,
                "on_chunk_complete": complete, "on_chunk_failed": failed}

    def _register_plan(self, job_id: str, plan: HistoricalSyncPlan, sequence_offset: int) -> None:
        if self.status_store is None:
            return
        for index, request in enumerate(plan.requests):
            self.status_store.upsert_chunk(DownloadChunkStatus(
                job_id=job_id, sequence=sequence_offset + index, instrument=request.instrument,
                start_ns=request.start_ns, end_ns=request.end_ns, status="QUEUED", attempts=0))
    ...
    def _resume_plan(self, job_id: str) -> tuple[HistoricalSyncPlan, tuple[int, ...]]:
        ...
        requests = tuple(HistoricalFetchRequest(self.source_name, chunk.instrument, job.timeframe,
                                                 chunk.start_ns, chunk.end_ns) for chunk in chunks)
    ...
    def run(self, *, spot_instrument: str, exchange: str, underlying: str, start, end,
            ...
            session_windows: Callable[[object], Iterable[SessionWindow]] | None = None,
            job_id: str | None = None, resume: bool = False) -> CashFutureHistoricalDownloadReport:
        ...
        try:
            if resume:
                ...
                if not plan.requests:
                    self.status_store.update_job(job_id, status="COMPLETE", error=None,
                ...
                result = self.executor.run(
                    ...
                    should_accept=self._chunk_is_complete,
                    **self._callbacks(job_id, sequence_numbers=sequence_numbers),
                )
            ...
            if self.status_store is not None and job_id is not None:
                ...
                self.status_store.create_job(
                    ...
                    requested_chunks=total_chunks, source=self.source_name, reset_existing=True)
                offset = 0
                self._register_plan(job_id, spot_plan, offset)
                offset += len(spot_plan.requests)
                for plan in future_plans:
                    self._register_plan(job_id, plan, offset)
                    offset += len(plan.requests)
            effective_skip = should_skip or self._chunk_is_complete
            sequence_offset = 0
            spot_result = self.executor.run(
                self.source, spot_plan, retry_attempts=retry_attempts, should_skip=effective_skip,
                should_accept=self._chunk_is_complete,
                **(self._callbacks(job_id, sequence_offset) if job_id else {}))
            ...
            for plan in future_plans:
                result = self.executor.run(
                    self.source, plan, retry_attempts=retry_attempts, should_skip=effective_skip,
                    should_accept=self._chunk_is_complete,
                    **(self._callbacks(job_id, sequence_offset) if job_id else {}))
            ...
            completed = spot_result.failed_request_index is None and all(r.failed_request_index is None for r in future_results) and len(future_results) == len(future_plans)
            if self.status_store is not None and job_id is not None:
                self.status_store.update_job(job_id, status="COMPLETE" if completed else "FAILED",
                                             error=None if completed else "historical download incomplete",
                                             catalog_count=self.catalog.count())
