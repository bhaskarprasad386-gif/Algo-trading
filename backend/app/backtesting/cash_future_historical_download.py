...
class CashFutureHistoricalDownloadService:
    ...
    def __init__(self, catalog: HistoricalCatalog, contract_catalog, *, source=None, executor=None, session_windows: Callable[[object], Iterable[SessionWindow]] | None = None, status_store: HistoricalDownloadStatusStore | None = None, instrument_master: InstrumentMaster | None = None):
    ...
    def _callbacks(self, job_id: str, sequence_offset: int = 0, sequence_numbers: tuple[int, ...] | None = None):
        ...
        def skip(index, request):
            ...
            persist_chunk_result(store, job_id=job_id, sequence=status_sequence(index), instrument=target.instrument, start_ns=target.start_ns, end_ns=target.end_ns, attempts=0, status="SKIPPED", fetched_records=0, inserted_records=0, expected_records=expected, actual_records=actual, missing_records=missing, first_missing_ns=first_missing)
        def complete(index, request, result, attempt):
            ...
            persist_chunk_result(store, job_id=job_id, sequence=status_sequence(index), instrument=target.instrument, start_ns=target.start_ns, end_ns=target.end_ns, attempts=attempt, status="COMPLETED", fetched_records=fetched, inserted_records=inserted, expected_records=expected, actual_records=actual, missing_records=missing, first_missing_ns=first_missing)
        def failed(index, request, error, attempts):
            from .download_status_progress import persist_chunk_result
            target = status_request(index, request)
            expected, actual, missing, first_missing = self._chunk_metrics(target)
            persist_chunk_result(store, job_id=job_id, sequence=status_sequence(index), instrument=target.instrument, start_ns=target.start_ns, end_ns=target.end_ns, attempts=attempts, status="FAILED", error=str(error), fetched_records=0, inserted_records=0, expected_records=expected, actual_records=actual, missing_records=missing, first_missing_ns=first_missing)
    ...
    def run(self, *, spot_instrument: str, exchange: str, underlying: str, start, end, timeframe: str = "1m", mode: str = "BOTH", retry_attempts: int = 3, should_skip: Callable[[object], bool] | None = None, session_windows: Callable[[object], Iterable[SessionWindow]] | None = None, job_id: str | None = None, resume: bool = False):
        ...
        try:
            ...
            spot_plan = self._plan_for_request(queue.spot.request)
            ...
            for item, plan in zip(queue.futures, future_plans):
                result = self.executor.run(self.source, plan, retry_attempts=retry_attempts, should_skip=effective_skip, should_accept=self._chunk_is_complete, **(self._callbacks(job_id, sequence_offset=sequence_offset) if job_id else {}))
