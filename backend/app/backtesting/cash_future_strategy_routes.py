...
class StrategyRunRequest(BaseModel):
    ...
    def validate_input_mode(self):
        ...
        if self.start_timestamp is not None and self.start_date is not None and self.start_timestamp.date() != self.start_date: raise ValueError("start_timestamp date must match start_date")
        if self.end_timestamp is not None and self.end_date is not None and self.end_timestamp.date() != self.end_date: raise ValueError("end_timestamp date must match end_date")
