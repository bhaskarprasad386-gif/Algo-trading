...
def instrument_key(
    ...
    return "|".join(
        [symbol.strip().upper(), segment.strip().upper(), instrument_type.strip().upper(), (contract_month or "SPOT").strip().upper()]
    )
...
def _validate_bar(payload: dict) -> None:
    ...
    if not all(isinstance(payload[field], (int, float)) and math.isfinite(float(payload[field])) for field in ("open", "high", "low", "close")):
    ...
    if lot_size is not None and (not isinstance(lot_size, int) or lot_size <= 0):
...
def local_coverage(db: Session, key: str, start: datetime, end: datetime) -> list[BacktestDataCoverage]:
    ...
    return list(db.scalars(select(BacktestDataCoverage).where(
...
def record_coverage(db: Session, *, key: str, symbol: str, segment: str, contract_month: str | None, start: datetime, end: datetime, row_count: int, data_version: str | None, source_hash: str | None, validated: bool = True) -> BacktestDataCoverage:
    ...
    item = db.scalar(select(BacktestDataCoverage).where(
        BacktestDataCoverage.instrument_key == key,
        BacktestDataCoverage.timeframe == TIMEFRAME_1M,
...
def iter_bars(db: Session, *, key: str, start: datetime, end: datetime, chunk_size: int = 5000):
    ...
    stmt = select(HistoricalMarketBar).where(
        HistoricalMarketBar.instrument_key == key,
        HistoricalMarketBar.timestamp >= start,
        HistoricalMarketBar.timestamp <= end,
    ).order_by(HistoricalMarketBar.timestamp).execution_options(yield_per=chunk_size)
