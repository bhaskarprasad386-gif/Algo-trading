...
class BacktestConfig:
    ...
    def __post_init__(self) -> None:
        for value, name in ((self.min_entry_gap,'min_entry_gap'),(self.exit_gap,'exit_gap'),(self.charges_per_trade,'charges_per_trade'),(self.funding_cost_per_trade,'funding_cost_per_trade')):
        ...
        if not isinstance(self.max_holding_days,int) or isinstance(self.max_holding_days,bool) or self.max_holding_days <= 0: raise ValueError('max_holding_days must be a positive integer')
...
def _executable_spread_profit(entry: CashFutureHistoryPoint, exit_point: CashFutureHistoryPoint) -> float:
    ...
    if any(price is None or float(price)<=0 for price in prices): raise ValueError('bid_ask execution requires entry cash_ask/future_bid and exit cash_bid/future_ask')
    return ((float(exit_point.cash_bid)-float(entry.cash_ask))+(float(entry.future_bid)-float(exit_point.future_ask)))*entry.lot_size
...
def run_backtest(points: Iterable[CashFutureHistoryPoint], config: BacktestConfig, cancel_check: Callable[[], bool] | None = None) -> dict:
    ...
    for point in points:
        if cancel_check is not None and cancel_check(): return {'status':'cancelled','trade_count':len(trades),'wins':sum(1 for t in trades if t['net_profit']>0),'losses':sum(1 for t in trades if t['net_profit']<=0),'net_profit':equity,'roi_pct':equity/total_capital*100.0 if total_capital else 0.0,'win_rate_pct':(sum(1 for t in trades if t['net_profit']>0)/len(trades)*100.0 if trades else 0.0),'invested_capital':total_capital,'max_drawdown':max_drawdown,'equity_curve':equity_curve,'trades':trades,'open_position':{'entry_time':entry.timestamp.isoformat(),'symbol':entry.symbol,'contract_month':entry.contract_month,'entry_gap':entry.gap,'lot_size':entry.lot_size} if entry is not None else None}
        ...
        if seen_contract is None: seen_contract=point.contract_month
        elif point.contract_month != seen_contract: raise ValueError('backtest input contains multiple contract months; run each contract separately')
