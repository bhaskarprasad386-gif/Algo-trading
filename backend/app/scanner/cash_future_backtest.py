...
def run_multi_contract_backtest_streaming(points:Iterable[CashFutureHistoryPoint],config:BacktestConfig)->dict:
    """Backtest an ordered contract stream without silently splitting contracts."""
    filtered=(point for point in points if config.contract_month is None or point.contract_month == config.contract_month)
    results=[]; last_contract=None; last_timestamp=None
