...
def _expiry_date(value: date | datetime) -> date:
    if isinstance(value, datetime):
        return _normalize_timestamp(value).date()
    if isinstance(value, date):
        return value
    raise ValueError("expiry_date must be a date or datetime")


def _is_expired(timestamp: datetime, expiry_date: date | datetime | None) -> bool:
    return expiry_date is not None and timestamp.date() >= _expiry_date(expiry_date)
...
class CashFutureHistoryPoint:
    ...
    def __post_init__(self) -> None:
        ...
        if self.expiry_date is not None:
            expiry = _expiry_date(self.expiry_date)
            if expiry < self.timestamp.date():
...
def analyze_historical_gap_outcomes(points: Iterable[CashFutureHistoryPoint], target_gap: float, tolerance: float = 0.0, contract_month: str | None = None, exit_gap: float = 0.0, max_holding_days: int = 30, charges_per_trade: float = 0.0, funding_cost_per_trade: float = 0.0) -> list[HistoricalGapOutcome]:
    ...
    for match in matches:
        ...
        later = [p for p in ordered if p.symbol == match.symbol and p.contract_month == match.contract_month and p.timestamp > match.timestamp]
        exit_point = None
        exit_reason = None
        for point in later:
            ...
            if expired:
                exit_point = point; exit_reason = "expiry"; break
            if holding_days >= max_holding_days:
                exit_point = point; exit_reason = "max_holding"; break
        if exit_point is None:
            outcomes.append(HistoricalGapOutcome(match, None, None, None, None, None, None)); continue
...
def build_graph_series(points: Iterable[CashFutureHistoryPoint], contract_month: str | None = None) -> dict[str, list]:
    ...
    return {"timestamps": [p.timestamp.isoformat() for p in selected], "cash": [p.cash_price for p in selected], "future": [p.future_price for p in selected], "gap": [p.gap for p in selected], "gap_pct": [p.gap_pct for p in selected], "net_profit": [p.net_profit for p in selected], "roi_pct": [p.roi_pct for p in selected]}
