...
class CashFutureConfig:
    ...
    def __post_init__(self) -> None:
        non_negative = {"min_gap": self.min_gap, "min_gap_pct": self.min_gap_pct, "min_net_profit": self.min_net_profit, "min_roi_pct": self.min_roi_pct, "min_margin": self.min_margin, "min_volume": self.min_volume, "min_oi": self.min_oi, "min_days_to_expiry": self.min_days_to_expiry, "charges": self.charges, "funding_cost": self.funding_cost, "history_days": self.history_days, "gap_match_tolerance": self.gap_match_tolerance, "graph_days": self.graph_days}
        if self.max_margin is not None: non_negative["max_margin"] = self.max_margin
        if self.max_days_to_expiry is not None: non_negative["max_days_to_expiry"] = self.max_days_to_expiry
        if self.max_bid_ask_spread_pct is not None: non_negative["max_bid_ask_spread_pct"] = self.max_bid_ask_spread_pct
        if self.max_cash_bid_ask_spread_pct is not None: non_negative["max_cash_bid_ask_spread_pct"] = self.max_cash_bid_ask_spread_pct
        for name, value in non_negative.items():
            try: finite = math.isfinite(value)
            except TypeError as exc: raise ValueError(f"{name} must be a finite number") from exc
            if not finite: raise ValueError(f"{name} must be a finite number")
            if value < 0: raise ValueError(f"{name} cannot be negative")
        if self.max_margin is not None and self.max_margin < self.min_margin: raise ValueError("max_margin cannot be below min_margin")
        if self.max_days_to_expiry is not None and self.max_days_to_expiry < self.min_days_to_expiry: raise ValueError("max_days_to_expiry cannot be below min_days_to_expiry")
...
def _bid_ask_spread_pct(bid: Optional[float], ask: Optional[float], ltp: float) -> Optional[float]:
    ...
    if not math.isfinite(bid) or not math.isfinite(ask) or not math.isfinite(ltp): return None
    if bid <= 0 or ask <= 0 or ltp <= 0 or ask < bid: return None
    return (ask - bid) / ltp * 100.0

def _ltp_inside_quote(bid: Optional[float], ask: Optional[float], ltp: float) -> bool:
    if bid is None or ask is None or bid <= 0 or ask <= 0 or ltp <= 0: return True
    if not math.isfinite(bid) or not math.isfinite(ask) or not math.isfinite(ltp): return False
...
def calculate_cash_future(cash: CashQuote, future: FutureQuote, config: CashFutureConfig) -> CashFutureResult:
    ...
    cash_spread_pct = _bid_ask_spread_pct(cash.bid, cash.ask, cash.ltp); future_spread_pct = _bid_ask_spread_pct(future.bid, future.ask, future.ltp)
    ...
    if cash.ask is not None and future.bid is not None and cash.ask > 0 and future.bid > 0:
        cash_execution_price = cash.ask; future_execution_price = future.bid; executable_gap = future.bid - cash.ask; executable_gap_pct = executable_gap / cash.ask * 100.0
    profit_gap = executable_gap if executable_gap is not None else gap
    gross = round(profit_gap * future.lot_size, 2); net = round(gross - config.charges - config.funding_cost, 2)
    cash_capital_price = cash_execution_price if cash_execution_price is not None else cash.ltp
    deployed = round(cash_capital_price * future.lot_size + future.margin_required, 2); roi = round((net / deployed * 100.0) if deployed > 0 else 0.0, 6)
    reasons: list[str] = []
    if cash.symbol.strip().upper() != future.symbol.strip().upper(): reasons.append("symbol_mismatch")
    threshold_gap = executable_gap if executable_gap is not None else gap; threshold_gap_pct = executable_gap_pct if executable_gap_pct is not None else gap_pct
