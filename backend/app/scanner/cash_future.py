...
def _bid_ask_spread_pct(bid: Optional[float], ask: Optional[float], ltp: float) -> Optional[float]:
    if bid is None or ask is None:
        return None
    if not math.isfinite(bid) or not math.isfinite(ask) or not math.isfinite(ltp):
        return None
...
def calculate_cash_future(cash: CashQuote, future: FutureQuote, config: CashFutureConfig) -> CashFutureResult:
    ...
    if cash.bid is not None and cash.ask is not None and (cash.bid <= 0 or cash.ask <= 0 or cash.ask < cash.bid):
        reasons.append("invalid_cash_bid_ask")
    if future.bid is not None and future.ask is not None and (future.bid <= 0 or future.ask <= 0 or future.ask < future.bid):
